...
def main():
    ...
    cap = cv2.VideoCapture(1)  # 替换为0使用摄像头
    if not cap.isOpened():
    ...
    # 抬手接续：记录上笔结束的画布坐标，新笔触时平移对齐
    stroke_end_pos = None      # 上一笔在画布中的最后一个点
    canvas_offset  = (0, 0)   # cur_pos → 画布坐标的偏移量
    ...
    try:
        while True:
            ...
            if prev_writing and not is_writing:
                ...
                if len(valid) > 5:
                    otc.end_trace()
                    stroke_end_pos = valid[-1]
                trajectory.clear()
                prev_pos = None
                hist_pts.clear()
            elif not prev_writing and is_writing:
                # 落笔：计算偏移，让新笔触从上笔结束处接续
                if stroke_end_pos is not None and cur_pos != (0, 0):
                    canvas_offset = (
                        stroke_end_pos[0] - cur_pos[0],
                        stroke_end_pos[1] - cur_pos[1],
                    )
                trajectory.clear()
                prev_pos = None
                hist_pts.clear()
            ...
            # 记录轨迹（cur_pos 经偏移后映射到画布坐标）
            if is_writing and cur_pos != (0, 0):
                cp = (cur_pos[0] + canvas_offset[0],
                      cur_pos[1] + canvas_offset[1])
                if prev_pos is not None:
                    sp = smart_smooth(cp, prev_pos, hist_pts)
                    prev_pos = sp
